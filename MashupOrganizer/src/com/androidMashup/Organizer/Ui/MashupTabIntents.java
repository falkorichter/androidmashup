...
public class MashupTabIntents extends Activity implements OnClickListener,
		OnItemClickListener, android.content.DialogInterface.OnClickListener,
		OnItemLongClickListener, IMashupActivity {

	private class EnabledContextMenuListener implements
		...
		public void onCreateContextMenu(ContextMenu menu, View v,
			...
			intentAction = myApp.enabledIntentsCursor
					.getString(myApp.enabledIntentsCursor
							.getColumnIndex(MashupProvider.INTENT_ACTION));

			SubMenu sub = menu.addSubMenu(2, 0, 1,
					"installed Apps to this Intent");
			Intent intent = new Intent(intentAction);
			sub.addIntentOptions(Menu.NONE, 0, 0, getComponentName(), null,
					intent, 0, null);
		}
	}
	...
	private static final int	CONTEXT_MENU_DISABLE	= 1;
	private static final int	CONTEXT_MENU_MASHUP		= 2;
	private static final int	CONTEXT_MENU_OPEN		= 3;
	...
	private void mashup(Cursor cursor) {
		intentTitle = cursor.getString(cursor
				.getColumnIndex(MashupProvider.INTENT_TITLE));
		intentAction = cursor.getString(cursor
				.getColumnIndex(MashupProvider.INTENT_ACTION));
	...
	private void mashup(String title, String action) {
		...
		List<ResolveInfo> mashupList = mPackageManager.queryIntentActivities(
				queryIntent, 0);
	...
	private void nativeMashup(Cursor cursor) {
		...
		try {
			...
			Toast.makeText(getApplicationContext(),
					"no applications for this mashup installed",
					Toast.LENGTH_LONG).show();
		}
	...
	public void onClick(DialogInterface dialog, int which) {
		if (dialog == descriptionIntentDialog
				&& which == DialogInterface.BUTTON_POSITIVE) {
			mashup(myApp.enabledIntentsCursor);
		}
		if (dialog == enableDialog && which == DialogInterface.BUTTON_POSITIVE) {
			long intentId = myApp.availableIntentsCursor
					.getLong(myApp.availableIntentsCursor
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		enabledIntentsAdapter = new SimpleCursorAdapter(this,
				R.layout.installed_intents_view, myApp.enabledIntentsCursor,
				from1, to1);
		...
		availableIntentsAdapter = new SimpleCursorAdapter(this,
				R.layout.available_intents_view, myApp.availableIntentsCursor,
				from2, to2);
	...
	public void onItemClick(AdapterView<?> adapter, View view, int pos,
		...
		if (adapter == enabledIntentsListView) {
			...
			String intentDescription = myApp.enabledIntentsCursor
					.getString(myApp.enabledIntentsCursor
							.getColumnIndex(MashupProvider.APPLICATION_DESCRIPTION));
			intentTitle = myApp.enabledIntentsCursor
					.getString(myApp.enabledIntentsCursor
							.getColumnIndex(MashupProvider.INTENT_TITLE));
			...
			descriptionIntentDialog = alertBuilder
					...
					.setPositiveButton("fire up this mashup!", this)
					.setMessage(intentDescription).setCancelable(true).create();
			descriptionIntentDialog.show();
		} else if (adapter == availableIntentsListView) {
			myApp.availableIntentsCursor.moveToPosition(pos);
			CharSequence intentDescripttion = myApp.availableIntentsCursor
					.getString(myApp.availableIntentsCursor
							.getColumnIndex(MashupProvider.APPLICATION_DESCRIPTION));
			intentTitle = myApp.availableIntentsCursor
					.getString(myApp.availableIntentsCursor
							.getColumnIndex(MashupProvider.INTENT_TITLE));
			enableDialog = alertBuilder
					// build.setIcon(R.drawable.alert_dialog_icon);
					.setNegativeButton("done", this).setTitle(intentTitle)
					.setPositiveButton("enable", this).setMessage(
							intentDescripttion).setCancelable(true).create();
			enableDialog.show();
		}
	...
	public boolean onMenuItemSelected(int featureId, MenuItem item) {
		...
		case CONTEXT_MENU_DISABLE:
			long intentId = myApp.enabledIntentsCursor
					.getLong(myApp.enabledIntentsCursor
							.getColumnIndex(MashupProvider.APPLICATION_KEY_ROWID));
