...
public class MashupDbAdapter {
	...
	public static final String		RELATION_APPLICATION_ID				= "application_id";
	public static final String		RELATION_INTENT_ID					= "intent_id";
	...
	private static final String		DATABASE_CREATE_TABLE_INTENTS		= "create table " + DATABASE_INTENTS_TABLE
																			...
																			+ " integer not null default 0" + ");";
	private static final String		DATABASE_CREATE_TABLE_RELATION		= "create table " + DATABASE_RELATION_TABLE
																			+ " (" + RELATION_KEY_ROWID
																			+ " integer primary key autoincrement,"
																			+ RELATION_APPLICATION_ID
																			+ " integer not null," + RELATION_INTENT_ID
																			+ " integer not null);";
	...
	public boolean deleteApplication(long rowId) {
		return mDb.delete(DATABASE_INTENTS_TABLE, MashupProvider.APPLICATION_KEY_ROWID + "=" + rowId, null) > 0;
	}
	
	public boolean deleteApplicationByWebId(long webId) {
		return mDb.delete(DATABASE_INTENTS_TABLE, MashupProvider.APPLICATION_WEB_ID + "=" + webId, null) > 0;
	}
	...
	public boolean deleteIntent(long rowId) {
		return mDb.delete(DATABASE_INTENTS_TABLE, MashupProvider.INTENT_KEY_ROWID + "=" + rowId, null) > 0;
	}
	
	public boolean deleteIntentByWebId(long webId) {
		return mDb.delete(DATABASE_INTENTS_TABLE, MashupProvider.INTENT_WEB_ID + "=" + webId, null) > 0;
	}
	
	public Cursor fetchAllApplications() {
		return mDb
				.query(DATABASE_APPLICATIONS_TABLE, new String[] { MashupProvider.APPLICATION_APK_URL, MashupProvider.APPLICATION_DEVELOPER_EMAIL, MashupProvider.APPLICATION_DEVELOPER_URL, MashupProvider.APPLICATION_ICON, MashupProvider.APPLICATION_INSTALLED, MashupProvider.APPLICATION_KEY_ROWID, MashupProvider.APPLICATION_NAME, MashupProvider.APPLICATION_PACKAGE, MashupProvider.APPLICATION_URL, MashupProvider.APPLICATION_WEB_ID }, null, null, null, null, null);
	}
	
	public Cursor fetchAllIntents() {
		return mDb
				.query(DATABASE_INTENTS_TABLE, new String[] { MashupProvider.INTENT_ACTION, MashupProvider.INTENT_DESCRIPTION, MashupProvider.INTENT_ENABLED, MashupProvider.INTENT_ICON, MashupProvider.INTENT_KEY_ROWID, MashupProvider.INTENT_TITLE, MashupProvider.INTENT_WEB_ID }, null, null, null, null, null);
	...
	public Cursor fetchIntent(long rowId) throws SQLException {
		...
		mDb
				.query(true, DATABASE_INTENTS_TABLE, new String[] { MashupProvider.INTENT_KEY_ROWID, MashupProvider.INTENT_ACTION, MashupProvider.INTENT_DESCRIPTION }, MashupProvider.INTENT_KEY_ROWID
																																										+ "="
	...
	public ArrayList<MashupApplication> getAllApplications() {
		...
		if (all.moveToFirst()) {
			...
			int apkUrlColumn = all.getColumnIndex(MashupProvider.APPLICATION_APK_URL);
			int developerEmailColumn = all.getColumnIndex(MashupProvider.APPLICATION_DEVELOPER_EMAIL);
			int iconColumn = all.getColumnIndex(MashupProvider.APPLICATION_ICON);
			int installedColumn = all.getColumnIndex(MashupProvider.APPLICATION_INSTALLED);
			int idColumn = all.getColumnIndex(MashupProvider.APPLICATION_KEY_ROWID);
			int nameColumn = all.getColumnIndex(MashupProvider.APPLICATION_NAME);
			int packageColumn = all.getColumnIndex(MashupProvider.APPLICATION_PACKAGE);
			int applicationUrlColumn = all.getColumnIndex(MashupProvider.APPLICATION_URL);
			int applicationPackageColumn = all.getColumnIndex(MashupProvider.APPLICATION_PACKAGE);
			int webIdColumn = all.getColumnIndex(MashupProvider.APPLICATION_WEB_ID);
			
			do {
				MashupApplication application = new MashupApplication();
				
				application.apkUrl = all.getString(apkUrlColumn);
				application.developerEmail = all.getString(developerEmailColumn);
				...
				application.applicationPackage = all.getString(applicationPackageColumn);
	...
	public ArrayList<MashupIntent> getAllIntents() {
		...
		if (all.moveToFirst()) {
			...
			int descriptionColumn = all.getColumnIndex(MashupProvider.INTENT_DESCRIPTION);
			int enabledColumn = all.getColumnIndex(MashupProvider.INTENT_ENABLED);
	...
	public long insertMashupApplication(MashupApplication app) {
		...
		initialValues.put(MashupProvider.APPLICATION_PACKAGE, app.applicationPackage);
		initialValues.put(MashupProvider.APPLICATION_DEVELOPER_EMAIL, app.developerEmail);
		initialValues.put(MashupProvider.APPLICATION_DEVELOPER_URL, app.developerUrl);
	...
	public long insertMashupIntent(MashupIntent intent) {
		...
		initialValues.put(MashupProvider.INTENT_DESCRIPTION, intent.description);
	...
	public int markAsInstalled(String packageName, String name) {
		...
		String query = MashupProvider.APPLICATION_PACKAGE + "='" + packageName + "' AND "
						+ MashupProvider.APPLICATION_NAME + "='" + name + "'";
		int updates = mDb.update(DATABASE_APPLICATIONS_TABLE, args, query, null);
		if (updates != 0) {
			Log.i("mark as installed application with packageName: '" + packageName + "' and name: " + name);
		}
		return updates;
	...
	public Cursor provideApplications(String[] projection, String selection, String[] selectionArgs, String sortOrder) {
		return mDb
				.query(DATABASE_APPLICATIONS_TABLE, projection, selection, selectionArgs, null, null, null, sortOrder);
	}
	
	public Cursor provideIntents(String[] projection, String selection, String[] selectionArgs, String sortOrder) {
		return mDb.query(DATABASE_INTENTS_TABLE, projection, selection, selectionArgs, null, null, null, sortOrder);
	}
	...
	public int updateOrInsertMashupApplication(MashupApplication app) {
		...
		args.put(MashupProvider.APPLICATION_DEVELOPER_EMAIL, app.developerEmail);
		args.put(MashupProvider.APPLICATION_DEVELOPER_URL, app.developerUrl);
		
		if (mDb.update(DATABASE_APPLICATIONS_TABLE, args, MashupProvider.APPLICATION_WEB_ID + "=" + app.webId, null) == 0) {
			mDb.insert(DATABASE_APPLICATIONS_TABLE, null, args);
			Log.i("inserting application '" + app.name + "' with webId " + app.webId);
			return 1;
		}
		Log.i("updating application '" + app.name + "' with webId " + app.webId);
		return 0;
	}
	...
	public int updateOrInsertMashupIntent(MashupIntent intent) {
		...
		if (mDb.update(DATABASE_INTENTS_TABLE, args, MashupProvider.INTENT_WEB_ID + "=" + intent.webId, null) == 0) {
			mDb.insert(DATABASE_INTENTS_TABLE, null, args);
			Log.i("inserting intent '" + intent.title + "' with webId " + intent.webId);
			return 1;
		}
		Log.i("updated intent '" + intent.title + "' with webId " + intent.webId);
		return 0;
	}
	
	public int updateOrInsertRelation(long intentId, long applicationId) {
		ContentValues initialValues = new ContentValues();
		initialValues.put(RELATION_INTENT_ID, intentId);
		initialValues.put(RELATION_APPLICATION_ID, applicationId);
		
		String query = RELATION_INTENT_ID + "=" + intentId + " AND " + RELATION_APPLICATION_ID + "=" + applicationId;
		Cursor entryExists = mDb
				.query(true, DATABASE_RELATION_TABLE, new String[] { RELATION_INTENT_ID, RELATION_APPLICATION_ID }, query, null, null, null, null, null);
